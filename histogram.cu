...
__global__ void histogram_i32_shared_kernel(int *a, int *y, int N, int num_bins) {
...
void histogram_i32_shared(int *d_a, int *d_y, int N, int num_bins) {
  ...
  histogram_i32_shared_kernel<<<grid_size, block_size, shared_mem_size>>>(d_a, d_y, N, num_bins);
}
...
int main() {
  ...
  for (int i = 0; i < N; i++) {
    input_data[i] = i % HISTOGRAM_SIZE;  // This will create uniform distribution
  }
  ...
  cudaMemcpy(histogram_result.get(), hist_gpu.get(), HISTOGRAM_SIZE * sizeof(int), cudaMemcpyDeviceToHost);
  ...
  histogram_i32x4_kernel<<<grid_vec4, block_vec4>>>(input_gpu.get(), hist_vec4_gpu.get(), N);
  cudaDeviceSynchronize();
  end = std::chrono::high_resolution_clock::now();
  
  double time_vec4 = std::chrono::duration<double, std::milli>(end - start).count();
  printf("histogram_i32x4_kernel execution time: %.3f ms\n", time_vec4);
  
  // Copy result back to CPU
  cudaMemcpy(histogram_result_vec4.get(), hist_vec4_gpu.get(), HISTOGRAM_SIZE * sizeof(int), cudaMemcpyDeviceToHost);
  
  // Test shared memory histogram kernel
  printf("\n=== Testing histogram_i32_shared_kernel ===\n");
  
  start = std::chrono::high_resolution_clock::now();
  histogram_i32_shared(input_gpu.get(), hist_shared_gpu.get(), N, HISTOGRAM_SIZE);
  cudaDeviceSynchronize();
  end = std::chrono::high_resolution_clock::now();
  
  double time_shared = std::chrono::duration<double, std::milli>(end - start).count();
  printf("histogram_i32_shared_kernel execution time: %.3f ms\n", time_shared);
  
  // Copy result back to CPU
  cudaMemcpy(histogram_result_shared.get(), hist_shared_gpu.get(), HISTOGRAM_SIZE * sizeof(int), cudaMemcpyDeviceToHost);
