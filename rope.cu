...
__global__ void rope_f32x4_pack_kernel(float *x, float *out, int seq_len,
  ...
  // First complex pair: indices token_idx*2 and token_idx*2+1 in the original
  // hidden_size
  ...
  // Second complex pair: indices token_idx*2+2 and token_idx*2+3 in the
...
void launch_rope_f32(UnifiedPtr<float> input, UnifiedPtr<float> output,
...
void launch_rope_f32_v2(UnifiedPtr<float> input, UnifiedPtr<float> output,
                        int seq_len, int hidden_size) {
  int N = hidden_size / 2;
  dim3 grid(seq_len);
  dim3 block(N);
...
void launch_rope_f32_pack(UnifiedPtr<float> input, UnifiedPtr<float> output,
                          int seq_len, int hidden_size) {
  int N = hidden_size / 4;
  int total_float4_pairs = seq_len * N;
  dim3 grid((total_float4_pairs + BLOCK_SIZE - 1) / BLOCK_SIZE);
  dim3 block(BLOCK_SIZE);
  rope_f32x4_pack_kernel<<<grid, block>>>(input.get(), output.get(), seq_len,
                                          N);
  // Check for kernel launch errors
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) {
...
int main() {
  ...
  printf(
      "Testing ROPE kernels with seq_len=%d, hidden_size=%d, "
      "total_elements=%d\n",
  ...
  cudaMemcpy(d_x.get(), h_x.data(), total_elements * sizeof(float),
  ...
  cudaMemcpy(h_out1.data(), d_out1.get(), total_elements * sizeof(float),
             cudaMemcpyDeviceToHost);

  bool test1_passed =
      check_approx_equal(h_out_ref.data(), h_out1.data(), total_elements);
  ...
  cudaDeviceSynchronize();
  cudaMemcpy(h_out2.data(), d_out2.get(), total_elements * sizeof(float),
             cudaMemcpyDeviceToHost);

  bool test2_passed =
      check_approx_equal(h_out_ref.data(), h_out2.data(), total_elements);
  ...
  cudaDeviceSynchronize();
  cudaMemcpy(h_out3.data(), d_out3.get(), total_elements * sizeof(float),
             cudaMemcpyDeviceToHost);

  bool test3_passed =
      check_approx_equal(h_out_ref.data(), h_out3.data(), total_elements);
