...
int main() {
  ...
  UnifiedPtr<float> input(row * col, DEVICE::CUDA);
  UnifiedPtr<float> output(col * row, DEVICE::CUDA);
  for (int i = 0; i < row; i++) {
    for (int j = 0; j < col; j++) {
      input[i * col + j] = i * col + j;
    }
  }

  auto st = std::chrono::high_resolution_clock::now();
  launch_transposeNaive(input.get(), output.get(), row, col);
  ...
  launch_transposeSharedMem(input.get(), output.get(), row, col);
  cudaDeviceSynchronize();
