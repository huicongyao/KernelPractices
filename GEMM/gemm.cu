...
__global__ void sgemm_sliced_k_f32_dbuf_kernel(const float *__restrict__ a,
  ...
  // 2x2x32x32x4B = 16KB shared memory
...
template <const int BM = 32, const int BN = 32, const int BK = 32>
__global__ void sgemm_sliced_k_f32x4_dbuf_kernel(float *a, float *b, float *c,
                                                 int M, int N, int K) {
...
#pragma unroll
    for (int k = 0; k < BK; k++) {
      ...
      int comp_smem_b_n = load_smem_b_n;
      sum[0] +=
          s_a[cur_smem][comp_smem_a_m][k] * s_b[cur_smem][k][comp_smem_b_n];
      sum[1] +=
          s_a[cur_smem][comp_smem_a_m][k] * s_b[cur_smem][k][comp_smem_b_n + 1];
      sum[2] +=
          s_a[cur_smem][comp_smem_a_m][k] * s_b[cur_smem][k][comp_smem_b_n + 2];
      sum[3] +=
          s_a[cur_smem][comp_smem_a_m][k] * s_b[cur_smem][k][comp_smem_b_n + 3];
    }

    int load_gmem_a_k = bk * BK + load_smem_a_k;
    int load_gmem_a_addr = load_gmem_a_m * K + load_gmem_a_k;
    // float4 a_val = FLOAT4(a[load_gmem_a_addr]);
    FLOAT4(s_a[nxt_smem][load_smem_a_m][load_smem_a_k]) =
        FLOAT4(a[load_gmem_a_addr]);
    int load_gmem_b_k = bk * BK + load_smem_b_k;
    int load_gmem_b_addr = load_gmem_b_k * N + load_gmem_b_n;
    // float4 b_val = FLOAT4(b[load_gmem_b_addr]);
    FLOAT4(s_b[nxt_smem][load_smem_b_k][load_smem_b_n]) =
        FLOAT4(b[load_gmem_b_addr]);
    __syncthreads();
  }
...
#pragma unroll
  ...
  int store_gmem_c_n = load_gmem_b_n;
  int store_gmem_c_addr = store_gmem_c_m * N + store_gmem_c_n;
  FLOAT4(c[store_gmem_c_addr]) = FLOAT4(sum[0]);
}

template <const int BM = 64, const int BN = 64, const int BK = 16,
          const int TM = 4, const int TN = 4>
__global__ void sgemm_t_4x4_sliced_k_f32x4_dbuf_kernel(
  ...
  // 256 threads, each thread reads 4 elements
  ...
  // Load first buffer
  {
    int load_gmem_a_k = 0 * BK + load_smem_a_k;
    int load_gmem_a_addr = load_gmem_a_m * K + load_gmem_a_k;
    FLOAT4(s_a[0][load_smem_a_m][load_smem_a_k]) =
        __ldg(reinterpret_cast<const float4 *>(&a[load_gmem_a_addr]));
    int load_gmem_b_k = 0 * BK + load_smem_b_k;
    int load_gmem_b_addr = load_gmem_b_k * N + load_gmem_b_n;
    FLOAT4(s_b[0][load_smem_b_k][load_smem_b_n]) =
        __ldg(reinterpret_cast<const float4 *>(&b[load_gmem_b_addr]));
    __syncthreads();
  }

  float r_c[TM][TN] = {0.0f};
  float r_load_a[4] = {0.0f};
  float r_load_b[4] = {0.0f};
  for (int bk = 1; bk < (K + BK - 1) / BK; bk++) {
    int cur_smem = (bk - 1) & 1;
    int nxt_smem = bk & 1;
...
#pragma unroll
    for (int k = 0; k < BK; k++) {
      // Each thread computes (4x4) elements in BM * BN (64x64) tile
#pragma unroll
      for (int m = 0; m < TM; m++) {
#pragma unroll
        for (int n = 0; n < TN; n++) {
    ...
    FLOAT4(s_a[nxt_smem][load_smem_a_m][load_smem_a_k]) = FLOAT4(r_load_a[0]);
    FLOAT4(s_b[nxt_smem][load_smem_b_k][load_smem_b_n]) = FLOAT4(r_load_b[0]);
    __syncthreads();
  }
...
__global__ void sgemm_t_8x8_sliced_k_f32x4_kernel(float *a, float *b, float *c,
                                                  int M, int N, int K) {
  // [1] Block Tile: A 16x16 block processes a 128x128 target tile in C
  // [2] Thread Tile: Each thread computes TM*TN (8*8) elements, increasing
  // compute density [3] K Tile: Partition K into BK-sized tiles, iterate
  // (K+BK-1)/BK times,
  //             each iteration computes partial multiply-accumulate for TM*TN
  //             elements
  // [4] Vectorize: Reduce load and store instructions using float4
  ...
  // 0. First compute shared memory indices
  // Index mapping between tid and smem s_a[BM][BK] BM=128 BK=8 row-major read A
  // row-major For s_a, each row has 8 elements, each thread reads 4 elements,
  // needs 2 threads per row Total 128 rows, requires 128x2 = 256 threads
  // exactly
  ...
  // Index mapping between tid and smem s_b[BK][BN] BK=8 BN=128 row-major read B
  // row-major For s_b, each row has 128 elements, each thread reads 4 elements,
  // needs 32 threads per row Total 8 rows, requires 32x8 = 256 threads
  int load_smem_b_k = tid / 32;        // tid/32, row of s_b 256/32=8 rows 0~7
  int load_smem_b_n = (tid % 32) * 4;  // (tid % 32) * 4, col of s_b 0,4,...,124
  // 1. Then compute global memory indices
  // Elements to load into s_a correspond to rows in global memory A
  // Each block is responsible for a BM*BN tile in C
  int load_gmem_a_m = by * BM + load_smem_a_m;  // global row of a and c
  int load_gmem_b_n = bx * BN + load_smem_b_n;  // global col of b and c

  float r_c[TM][TN] = {0.0};  // 8x8
  // 2. First partition K into BK-sized tiles
  for (int bk = 0; bk < (K + BK - 1) / BK; ++bk) {
    // Load data to shared memory smem s_a BM*BK 128*8 vectorize float4
    int load_gmem_a_k = bk * BK + load_smem_a_k;  // global col of a
    int load_gmem_a_addr = load_gmem_a_m * K + load_gmem_a_k;
    FLOAT4(s_a[load_smem_a_m][load_smem_a_k]) = FLOAT4(a[load_gmem_a_addr]);
    // Load data to shared memory smem s_b BK*BN 8*128 vectorize float4
...
#pragma unroll
    for (int k = 0; k < BK; k++) {
// 3. Each thread computes TM*TN (8x8) elements in BM*BN (128x128) tile
#pragma unroll
      for (int m = 0; m < TM; m++) {
#pragma unroll
        for (int n = 0; n < TN; n++) {
          ...
          int comp_smem_a_m =
              ty * TM + m;  // 128*8 128/TM(8)=16 16 threads in M direction
          int comp_smem_b_n =
              tx * TN + n;  // 8*128 128/TN(8)=16 16 threads in N direction
...
// TODO(huicongyao): this kernel uses too many registers
template <const int BM = 128, const int BN = 128, const int BK = 8,
...
__global__ void sgemm_t_8x8_sliced_k_f32x4_bcf_dbuf_kernel(
  ...
  // 1) Main loop starts from bk = 1
  //    First data loading happens before the main loop, last computation
  //    happens after the main loop This is determined by the pipeline
  //    characteristics
  // 2) Since computation and next memory access use different Shared Memory
  // buffers
  //    Only one __syncthreads() is needed per iteration in the main loop
  // 3) Since GPU doesn't support out-of-order execution like CPU
  //    In the main loop, we first load data from Global Memory to registers for
  //    next iteration Then perform current computation, then write the loaded
  //    data to Shared Memory This way, when LDG instruction loads from Global
  //    Memory It doesn't affect the launch and execution of subsequent FFMA and
  //    other compute instructions Achieving the purpose of Double Buffering
  ...
  // Load first tiles
  {
    ...
    FLOAT4(r_load_b[0]) =
        __ldg(reinterpret_cast<const float4 *>(&b[load_b_gmem_addr]));
...
void launch_sgemm_sliced_k_f32_dbuf_kernel(const float *__restrict__ a,
                                           const float *__restrict__ b,
...
void launch_sgemm_sliced_k_f32x4_dbuf_kernel(float *a, float *b, float *c,
                                             int M, int N, int K) {
  constexpr int BM = 32, BN = 32, BK = 32;
  dim3 block(BN / 2, BM / 2);
  dim3 grid((N + BN - 1) / BN, (M + BM - 1) / BM);
  sgemm_sliced_k_f32x4_dbuf_kernel<BM, BN, BK>
      <<<grid, block>>>(a, b, c, M, N, K);
}

void launch_sgemm_t_4x4_sliced_k_f32x4_dbuf_kernel(float *a, float *b, float *c,
                                                   int M, int N, int K) {
  constexpr int BM = 64, BN = 64, BK = 16, TM = 4, TN = 4;

  dim3 block(BN / TN, BM / TM);
  dim3 grid((N + BN - 1) / BN, (M + BM - 1) / BM);
  sgemm_t_4x4_sliced_k_f32x4_dbuf_kernel<BM, BN, BK, TM, TN>
      <<<grid, block>>>(a, b, c, M, N, K);
}
...
void launch_sgemm_t_8x8_sliced_k_f32x4_bcf_dbuf_kernel(
    const float *__restrict__ a, const float *__restrict__ b,
