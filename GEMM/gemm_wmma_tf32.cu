...
void launch_sgemm_wmma_m16n16k8_mma4x2_warp2x4_stages_kernel(
    float *a, float *b, float *__restrict__ c, int M, int N, int K) {
  const int Na = M * K;
  const int Nb = K * N;
  constexpr int T = 256;
...
void benchmark_group_gemm(int M, int N, int K, int repeats = 10) {
  ...
  benchmark_gemm<
      decltype(launch_sgemm_wmma_m16n16k8_mma4x2_warp2x4_stages_kernel), float>(
      launch_sgemm_wmma_m16n16k8_mma4x2_warp2x4_stages_kernel, M, N, K,
      "stemm_wmma_stages_async_kernel", repeats);
  benchmark_gemm<
      decltype(launch_sgemm_wmma_m16n16k8_mma4x2_warp2x4_stages_dsmem_kernel), float>(
      launch_sgemm_wmma_m16n16k8_mma4x2_warp2x4_stages_dsmem_kernel, M, N, K,
      "stemm_wmma_stages_async_dsmem_kernel", repeats);
}
