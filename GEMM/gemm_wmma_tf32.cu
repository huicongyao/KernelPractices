...
#pragma unroll
  for (int k = 0; k < (K_STAGE - 1); ++k) {
    ...
    // cp.async访存模式有待优化（一次访存，同warp内线程访存并不连续）
...
#pragma unroll
        for (int i = 0; i < 2; ++i) {
          ...
          const float *load_smem_a_frag_ptr =
              (s_a + smem_sel * s_a_stage_offset +
               warp_smem_a_m * (BK + A_PAD) + 0);  // BK=WMMA_K=8
          wmma::load_matrix_sync(A_frag[i], load_smem_a_frag_ptr, BK + A_PAD);
        }
#pragma unroll
        for (int j = 0; j < 2; ++j) {
...
#pragma unroll
          for (int j = 0; j < 2; ++j) {
            wmma::mma_sync(C_frag[i + i_block][j + j_block], A_frag[i],
                           B_frag[j], C_frag[i + i_block][j + j_block]);
          }
        }
      }
    }
...
template <const int WMMA_M = 16, const int WMMA_N = 16, const int WMMA_K = 8,
          ...
          const int A_PAD = 0, const int B_PAD = 0, const int K_STAGE = 2,
          const bool BLOCK_SWIZZLE = false>
__global__ void sgemm_wmma_m16n16k8_mma2x4_warp4x4_stages_dsmem_kernel(
    const float *__restrict__ A, const float *__restrict__ B,
    float *__restrict__ C, const int M, const int N, const int K) {
  const int bx =
      (static_cast<int>(BLOCK_SWIZZLE)) * blockIdx.z * gridDim.x + blockIdx.x;
  ...
  constexpr int s_a_stage_offset = BM * (BK + A_PAD);
  constexpr int s_b_stage_offset = BK * (BN + B_PAD);
  // totally allocated K_STAGE * 12KB shared_mem
...
#pragma unroll
  for (int k = 0; k < (K_STAGE - 1); ++k) {
    ...
    const uint32_t base_smem_b_ptr_0 =
        ...
        (k * s_b_stage_offset + load_smem_b_k * (BN + B_PAD) + load_smem_b_n) *
            sizeof(float);
    const uint32_t load_smem_b_ptr_1 = base_smem_b_ptr_0 + 4 * sizeof(float);
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    printf("Usage: ./gemm_wmma_tf32 [profile|benchmark]\n");
    exit(1);
  }
